...
export function WalletTrackerBsc({
  price,
  apiKey = BSC_API_KEY,
  tokenAddress = TRDG_BSC_ADDRESS,
  ...
  const fetchWalletBalance = async (address: string) => {
    try {
      const url = `${CORS_PROXY}https://api.bscscan.com/api?module=account&action=tokenbalance&contractaddress=${tokenAddress}&address=${address}&tag=latest&apikey=${apiKey}`
  ...
  const fetchWalletTransfers = async (address: string) => {
    try {
      const url = `${CORS_PROXY}https://api.bscscan.com/api?module=account&action=tokentx&contractaddress=${tokenAddress}&address=${address}&page=1&offset=10000&startblock=0&endblock=999999999&sort=asc&apikey=${apiKey}`
  ...
  const fetchWalletData = async (address: string) => {
    ...
    try {
      ...
      if (address.toLowerCase() === BURN_WALLET_ADDRESS.toLowerCase()) {
        setWalletData({
          address: address,
          balance: "68,163,529,543,528,296",
          balanceUsd: "393,944.30",
          rewards: "20,649,121,154,233,436",
          rewardsUsd: "119,339.53",
        })
        setIsLoading(false)
        return
      }
